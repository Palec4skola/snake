...
printf("Adresa arg štruktúry: %p\n", &data);
  printf("Adresa mutexu: %p\n",&data->shared->players_mutex);
...
printf("Adresa SHARED_DATA štruktúry: %p\n", (void*)data);
...
int main(int argc, char * argv[]) {
  ...
  data->isRunning =1;
  printf("inicializuje sa mutex\n");
  pthread_mutex_init(&data->players_mutex, NULL);
  printf("Mutex inicializovany: %p\n",&data->players_mutex);
  data->game = &game;
  ...
  int newSocket;
    ...
    printf("mutex na clientovi: %p\n", &arg->shared->players_mutex);
    printf("arg MAIN: %p\n", &arg);
